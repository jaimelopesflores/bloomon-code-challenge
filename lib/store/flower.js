...
function add(flower) {
  ...
  if (!/[a-z]/g.test(name)) {
    return info({ module: 'flower-store', flower }, 'Flower name is not valid. Ignoring.');
  }
  if (!/S|L/.test(size)) {
    return info({ module: 'flower-store', flower }, 'Flower size is not valid. Ignoring.');
  }
  ...
  info({ module: 'flower-store', inMemoryHashMap }, 'Adding flower');
};
